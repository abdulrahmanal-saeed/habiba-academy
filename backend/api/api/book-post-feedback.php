...
$teacherScore = $teacherScoreRaw === '' ? null : max(0, min(100, (float)$teacherScoreRaw));
...
$speakingScore = $speakingScoreRaw === '' ? null : max(0, min(100, (float)$speakingScoreRaw));

$pdo->beginTransaction();
try {
    $pdo->prepare("DELETE FROM book_feedback WHERE submission_id = ?")->execute([$submissionId]);
    $ins = $pdo->prepare("INSERT INTO book_feedback (submission_id, teacher_id, feedback_type, feedback_text) VALUES (?, ?, ?, ?)");
    foreach ([
        ...
        if ($text !== '') {
            $ins->execute([$submissionId, $teacherId, $type, $text]);
        }
    }
