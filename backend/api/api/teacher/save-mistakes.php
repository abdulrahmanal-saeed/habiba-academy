...
if ($existingId > 0) {
    ...
    $stmt = $pdo->prepare("
        ...
        VALUES (?, ?, ?, NOW(), 1)
    ");
    $stmt->execute([$student_id, $mistake_tag, $note ?: null]);
...
try {
    ...
    $tax = $pdo->prepare("
        ...
        WHERE is_active = 1
          AND (
            LOWER(label_en) = LOWER(?)
            OR LOWER(code) = LOWER(?)
            OR LOWER(label_ar) = LOWER(?)
          )
        LIMIT 1
    ");
    ...
    if ($taxonomyId > 0) {
        ...
        ");
        $event->execute([$student_id, $taxonomyId, $mistake_tag, $note ?: null]);
    }
} catch (Throwable $e) {
    // Legacy mistake saving should never fail because taxonomy/event logging failed.
}

// Push to student — notify them about their new tracked mistake
try {
