...
// ── No CSRF here — multipart/form-data, we read it from $_POST instead ──────
// CSRF token still validated from POST body field
...
$mediaType = trim((string)($_POST['media_type'] ?? '')); // 'audio' | 'image'

// ── Audio upload (for lt_blocks listening sections) ─────────────────────────
if ($mediaType === 'audio') {
    ...
    if (!move_uploaded_file($f['tmp_name'], $dest)) {
        json_err('Failed to save audio file');
    }
...
// ── Image upload (for speaking prompt 2 picture description) ────────────────
if ($mediaType === 'image') {
    // Optional: save to speaking prompt setting key
    ...
    if (!move_uploaded_file($f['tmp_name'], $dest)) {
        json_err('Failed to save image file');
    }

    $path = 'assets/img/leveltest/' . $filename;

    // If a setting key was given, persist the path
