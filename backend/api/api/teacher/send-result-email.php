...
function score_row(string $ar, string $en, string $icon, int $raw, int $max, string $lvl): string {
    ...
    return "<tr style='border-bottom:1px solid #f0eaff'>
      ...
      <td style='padding:10px 14px;font-size:16px;font-weight:900;color:#7c3aed;text-align:center'>{$raw}/{$max}</td>
...
function skill_block(string $ar, string $en, string $icon, int $raw, int $max, string $lvl, array $bank_skill): string {
    ...
    return "
      <div style='border:1px solid #ede9ff;border-radius:10px;padding:18px 20px;margin-bottom:16px'>
        <div style='display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px'>
          ...
          <div style='text-align:center;flex-shrink:0'>
            <div style='font-size:22px;font-weight:900;color:#7c3aed;line-height:1'>{$raw}/{$max}</div>
            <div style='font-size:11px;color:#7c3aed;margin-top:2px'>{$label}</div>
          </div>
        </div>
        <div style='width:100%;background:#f0eaff;border-radius:4px;height:5px;margin-bottom:12px'>
          <div style='background:#7c3aed;border-radius:4px;height:5px;width:" . round($raw/$max*100) . "%'></div>
        </div>
...
$score_rows = score_row('الاستماع', 'Listening', '🎧', $listening_score, 30, $l_lvl)
            ...
            . score_row('الكتابة',  'Writing',   '✍️', $writing_score,   40, $w_lvl);
...
$skill_blocks = skill_block('الاستماع', 'Listening', '🎧', $listening_score, 30, $l_lvl, $bank['listening'])
              ...
              . skill_block('الكتابة',  'Writing',   '✍️', $writing_score,   40, $w_lvl, $bank['writing']);

$notes_section = '';
if ($notes) {
    $notes_html = htmlspecialchars($notes, ENT_QUOTES);
    $notes_section = "
      <div style='background:#f8f4ff;border-radius:10px;padding:18px 20px;margin-bottom:24px'>
        <div style='font-size:13px;font-weight:700;color:#7c3aed;margin-bottom:8px'>
          Teacher Notes · ملاحظات المعلمة
        </div>
...
<body style="margin:0;padding:0;background:#f0eaff;font-family:Tahoma,Arial,sans-serif">
<table width="100%" cellpadding="0" cellspacing="0" style="background:#f0eaff;padding:32px 0">
  <tr><td align="center">
    <table width="620" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:16px;overflow:hidden;max-width:620px;width:100%;box-shadow:0 4px 24px rgba(124,58,237,.1)">

      <!-- ── Header ── -->
      <tr>
        <td style="background:linear-gradient(135deg,#7c3aed 0%,#9f67f5 100%);padding:40px 28px;text-align:center;color:#fff">
          <div style="font-size:13px;opacity:.8;margin-bottom:6px;letter-spacing:.5px">HABIBA NABIL · حبيبة نبيل</div>
          ...
          <div style="margin-top:6px;font-size:13px;opacity:.7">Total · المجموع: {$total}/175 ({$overall_pct}%)</div>
        </td>
      </tr>
      ...
      <tr>
        <td style="padding:24px 28px 8px">
          ...
          <div style="font-size:13px;color:#888;margin-top:4px">Test date · تاريخ الاختبار: {$date_str}</div>
        </td>
      </tr>
      ...
      <tr>
        <td style="padding:12px 28px 0">
          <div style="font-size:13px;font-weight:700;color:#7c3aed;margin-bottom:10px;text-transform:uppercase;letter-spacing:.5px">
            Score Summary · ملخص النتائج
          </div>
          <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;border:1px solid #ede9ff;border-radius:10px;overflow:hidden">
            <thead>
              <tr style="background:#f8f4ff">
                <th style="padding:10px 14px;font-size:12px;color:#7c3aed;text-align:right;font-weight:700">Skill · المهارة</th>
                <th style="padding:10px 14px;font-size:12px;color:#7c3aed;text-align:center;font-weight:700">Score</th>
                <th style="padding:10px 14px;font-size:12px;color:#7c3aed;text-align:center;font-weight:700">Level · المستوى</th>
              </tr>
            </thead>
            <tbody>
              {$score_rows}
      ...
      <tr>
        <td style="padding:24px 28px 8px">
          <div style="font-size:13px;font-weight:700;color:#7c3aed;margin-bottom:14px;text-transform:uppercase;letter-spacing:.5px">
            Skills Detail · تفاصيل المهارات
          </div>
          {$skill_blocks}
        </td>
      </tr>
      ...
      <!-- ── Footer ── -->
      <tr>
        <td style="background:#f8f4ff;padding:24px 28px;text-align:center;border-top:1px solid #ede9ff">
          <div style="font-size:14px;font-weight:700;color:#7c3aed">Habiba Nabil · حبيبة نبيل</div>
          <div style="font-size:12px;color:#999;margin-top:4px">Arabic Language School · مدرسة اللغة العربية</div>
...
if (!$sent) {
    json_err('Failed to send email. Please check server mail configuration.');
}
