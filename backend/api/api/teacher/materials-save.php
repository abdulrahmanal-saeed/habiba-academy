...
require_once __DIR__ . '/../../lib/materials-library.php';
...
if ($sourceType === 'file') {
    ...
    if ($id > 0 && $sourceType === 'url' && $filePath !== '') {
        $filePath = '';
        $originalFilename = '';
        $mimeType = '';
        $fileSize = 0;
...
if ($id > 0) {
    ...
    if ($isPublished) {
        push_notify_student($pdo, $studentId, 'Material updated', $title, '/student/material.php?id=' . $id);
    }
...
if ($isPublished) {
    push_notify_student($pdo, $studentId, 'New material assigned', $title, '/student/material.php?id=' . $newId);
}
