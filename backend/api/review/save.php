<?php
// api/review/save.php — Teacher grades a student submission
...
foreach ($overrides as $row) {
    ...
    if ($sectionId === '' || !isset($sectionMap[$sectionId]) || empty($sectionMap[$sectionId]['auto_gradable'])) continue;
...
try {
    ...
    foreach ($scores as $row) {
        ...
        if ($verdict === 'correct')      { $score = $maxPoints; }
        elseif ($verdict === 'wrong')    { $score = 0.0; }
        else                             { $verdict = null; }
    ...
    $pdo->prepare("
        ...
        SET manual_score=?, total_score=?, teacher_note=?, review_status='reviewed', reviewed_at=NOW()
        WHERE id=?
    ")->execute([(int)round($manualScore), $totalScore, $teacherNote ?: null, $submissionId]);
