<?php
// api/teacher/lt-prompt-save.php — Save level test writing/speaking prompts
...
if ($type === 'writing') {
    ...
    if (!in_array($key, $allowedWritingKeys, true)) json_err('Invalid setting key: ' . $key);
...
if ($type === 'randomization') {
    ...
    $listeningBlocks = max(1, min(10, (int)($_POST['lt_random_listening_blocks_per_level'] ?? 2)));
    $readingBlocks   = max(1, min(10, (int)($_POST['lt_random_reading_blocks_per_level']  ?? 1)));
