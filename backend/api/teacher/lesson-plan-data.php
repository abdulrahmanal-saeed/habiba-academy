...
try {
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS lesson_plan_sessions (
        ...
        status           ENUM('planned','completed','skipped','rescheduled','absent','cancelled') NOT NULL DEFAULT 'planned',
...
$totalIncome     = 0.0;
foreach ($sessions as $s) {
    if (!in_array($s['status'], ['completed', 'absent'], true)) continue;
    $duration = (int)($s['duration_minutes'] ?? $contract['session_duration_minutes']);
    if ($duration <= 0) $duration = (int)$contract['session_duration_minutes'];
