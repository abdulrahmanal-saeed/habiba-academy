...
$find = $pdo->prepare("
    SELECT id FROM student_weak_words
...
if ($existingId > 0) {
    $pdo->prepare("UPDATE student_weak_words SET note = ?, is_active = 1 WHERE id = ? AND student_id = ?")
        ->execute([$note ?: null, $existingId, $student_id]);
    $wordId = $existingId;
} else {
    $pdo->prepare("INSERT INTO student_weak_words (student_id, word, note, created_at, is_active) VALUES (?, ?, ?, NOW(), 1)")
        ->execute([$student_id, $word, $note ?: null]);
