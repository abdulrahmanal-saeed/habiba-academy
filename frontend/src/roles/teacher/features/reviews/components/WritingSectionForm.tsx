...
interface WritingSectionFormProps {
  questions: WritingQuestion[]
  onChange: (questions: WritingQuestion[]) => void
}

const BLANK: WritingQuestion = {
  id: '',
  prompt: '',
  points: 15,
  sample_answer: '',
...
export const WritingSectionForm: FC<WritingSectionFormProps> = ({ questions, onChange }) => {
  const [draft, setDraft] = useState<WritingQuestion>(() => ({ ...BLANK, id: `w_${Date.now()}` }))
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="p-3 rounded-lg border border-dashed border-[var(--border)] space-y-2">
        <p className="text-xs font-semibold text-[var(--text-2)]">New Prompt</p>
        <textarea
        ...
        <textarea
          ...
          onChange={e => setDraft(d => ({ ...d, sample_answer: e.target.value }))}
        />
